...
function box_user_online($file_tpl = 'user_online') {
	...
	*/
	...
	$member_sql = $mysql->query("SELECT user_id, user_name, user_level, user_activated FROM ".$tb_prefix."user WHERE user_online = 1 AND user_timeout > ".$time_exit." ORDER BY user_timeout DESC");
	...
	while($r = $mysql->fetch_array($member_sql)){
		...
		$activated = $r['user_activated'];
		 if ($activated==1)
		 {
		         ...
		         elseif ($level == 4) $s = $c['bannedmember'];
		 }
		 else
		 {
		 $s = $c['inactivated'];
