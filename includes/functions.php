...
function m_get_data($type,$v,$field = '') {
	...
	elseif ($type == 'ALBUM') {
		...
		if (!$cached['album']['album_'.$v][$field]) {
			if ($v == 0 && $field == 'album_name') return "Coming soon";
	...
	elseif ($type == 'SINGER') {
		...
		if ($field == 'singer_name') {
			...
			if (!$c_name) {
				if ($v == -1) $c_name = "Coming soon (VN)";
				elseif ($v == -2) $c_name = "Coming soon (QT)";
...
function m_blog($r) {
	...
	$html = $tpl->assign_vars($html,
		array(
			...
			'song.URL'    =>    $mainURL.'/Play,'.$r['m_id'].','.name_on_bar($r['m_title'],1),
...
/////////////////////////////
	...
	$html = $tpl->assign_vars($t['info'],
		array(
			...
			'MEDIA_LINK'    =>    $mainURL.'/Play,'.$r['m_id'].','.name_on_bar($r['m_title'],1),
...
function m_play($r) {
	global $mysql,$tb_prefix,$tpl,$mediaFolder,$gvns;
	...
	while ($r2 = $mysql->fetch_array($q)) {
		if ($id) {
			$rx = $mysql->fetch_array($mysql->query("SELECT * FROM ".$tb_prefix."options_values WHERE m_id = '".$id."' AND option_id = '".$r2['option_id']."'"));
			if ($rx['option_value']) $r['m_option'] .= "<tr><td valign=top><b>".$r2['option_name']."</b></td><td>".$rx['option_value']."</td></tr>";;
		}
	}
	$cat_t2 = array();
	$qx = $mysql->query("SELECT cat_id, cat_name FROM ".$tb_prefix."cat WHERE cat_id IN (".$r['m_cat'].")");
	while ($cat_t = $mysql->fetch_array($qx))
		$cat_t2[] = "<a href='List,".$cat_t['cat_id']."'>".$cat_t['cat_name']."</a>";
	...
	$html = $tpl->assign_vars($html,array
		(
			...
			'CAT'		=>	$r['cat_name'],
			'OPTIONS'		=>	$r['m_option'],
			'LINK1'		=>	getlinks($r['m_linkr'],'/',1),
			'LINK2'		=>	getlinks($r['m_linkm'],'?',1),
			'LINK3'		=>	getlinks($r['m_linko'],'',1),
			'PICZ'	=>	"<img width='150px' src='".$r['m_img']."'>",
		)
	);
	...
	if ($r['m_url']) $html = $tpl->assign_blocks_content($html,
	 	array(
			'player'	=>	m_player($arr),
		)
	);
	else $html = $tpl->unset_block($html,array('trailer'));
	if ((!$r['m_album'])||($r['m_album'] == 0))
		$html = $tpl->unset_block($html,array('col'));
	if ((!$r['m_linkr'])&&(!$r['m_linkm'])&&(!$r['m_linko']))
		$html = $tpl->unset_block($html,array('down'));
	else {
		if (!$r['m_linkr']) $html = $tpl->unset_block($html,array('link1'));
		if (!$r['m_linkm']) $html = $tpl->unset_block($html,array('link2'));
		if (!$r['m_linko']) $html = $tpl->unset_block($html,array('link3'));
	}
...
function name_on_bar($string,$title) {
  $string = m_unhtmlchars(stripslashes(utf8_to_ascii($string)));
  $string = str_replace ( ' ', '-', $string );
  $string = str_replace ( '~', '', $string );
  $string = str_replace ( "!", "", $string );
  $string = str_replace ( "@", "", $string );
