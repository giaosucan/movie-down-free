...
if (!$isLoggedIn && !$_POST['showcomment']) die("<center><b>Have to login</b></center>");
$tpl =& new Template;
...
if ($_POST['showcomment'] && $_POST['media_id']) {
	...
	if (!$isLoggedIn) {
		$main = $tpl->assign_blocks_content($main,
			array(
				'write_comment'	=>	'<div><b><center>Have to login to write comment</center></b></div>',
			)
		);
...
elseif ($_POST['comment'] && $_POST['media_id'] && $_POST['comment_content']) {
	...
	if ($comment_content) {
		...
		if (!$mysql->num_rows($q) || m_check_level($_SESSION['user_id'])) {
			...
			$r = $mysql->fetch_array($q);
			$warn = "Wait <b>".($r['manage_timeout'] - NOW)."</b> second to write next comment";
		}
	}
	else $warn = "No message input";
