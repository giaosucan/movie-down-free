...
<br><input type="submit" value="upload">
	...
	case 'chmod':
		if(!empty($_POST['files']) && empty($_POST['mod']))
		{
			...
			Please enter the CHMOD value for selected items:  <input type="text" name="mod" value="'.get_rights(clean($_POST['files'][0])).'">
			<br><input type="checkbox" name="recurse" value="true" id="recurse"><label for="recurse"> Recursive CHMOD (chmods also all subdirectories and all files in subdirectories)</label>
...
if(!$nomain || $_REQUEST['act']=='main')
{
	...
	if(empty($_GET['p']) && !empty($_REQUEST['DIR'])) $_GET['p']='';
	else if(empty($_GET['p']) && empty($_REQUEST['DIR']) && isset($_SESSION['p'])) $_GET['p']=$_SESSION['p'];
...
<div id="debug" style="position: absolute; width: 200px; top:0px; left: 0px;"></div><table width="100%" cellpadding="0" cellspacing="0" border="0">
 <tr>
  <td class="main" bgcolor="#c3d9ff" valign="middle"><?
   if($_GET['sort']=='size')
  {
  	if(SHOW_DIRSIZE && !empty($dirs))
  	{
  		$dsizes=array();
  		foreach($dirs as $v) $dsizes[$v]=show_size($v,false);
  		if($_GET['order']=='asc') asort($dsizes,SORT_NUMERIC);
  		else arsort($dsizes,SORT_NUMERIC);
  		$dirs=array_keys($dsizes);
  	}

  	if(!empty($files))
  	{
  		$fsizes=array();
  		foreach($files as $v) $fsizes[$v]=show_size($v,false);
  		if($_GET['order']=='asc') asort($fsizes,SORT_NUMERIC);
  		else arsort($fsizes,SORT_NUMERIC);
  		$files=array_keys($fsizes);
  	}
  }

  if($_GET['sort']=='type')
  {
  	if(!empty($dirs)) natsort($dirs);

  	if(!empty($files))
  	{
  		foreach($files as $v) $ftypes[substr($v,($p=strrpos($v,'.'))!==false ? $p : strlen($v))][]=$v;
  		if($_GET['order']=='asc') ksort($ftypes);
  		else krsort($ftypes);
  		$files=array();
  		foreach($ftypes as $v)
  		{
  			natsort($v);
  			if($_GET['order']!='asc') $v=array_reverse($v);
  ...
  {
  	$s = strtolower($_GET['filter']);
  	$dirs_c = $dirs;
  	$files_c = $files;
  	$dirs=$files=array();

  	foreach($dirs_c as $v) if(strpos(strtolower(basename($v)), $s)!==false) $dirs[]=$v;
  	foreach($files_c as $v) if(strpos(strtolower(basename($v)), $s)!==false) $files[]=$v;
  }
  ...
  $per = 30; // per page
  $pages = ceil((sizeof($dirs)+sizeof($files))/$per);
  ...
  if($pages>1)
  {
  	...
  	$frame=Tools_PagesList::frame(7,$per,sizeof($dirs)+sizeof($files));
  	
  	/*for($i=1;$i<=$pages;$i++)
  	{
  		if($i==$page) $pg.='<b>'.$i.'</b> ';
  		else $pg.='<a href="'.urlReplaceArg($_SERVER['REQUEST_URI'],'page',$i).'">'.$i.'</a> ';
  	}*/
...
<form action="index.php" method="POST" onsubmit="return validate_act();">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" class="list">
   <tr height="24" bgcolor="#a2c4fe">
    <th id="sort_menu"><div><?=show_sort('Name','name','light')?> | <?=show_sort('Type','type','light')?></div></th>
    <th align="left" width="75">&nbsp;<?=show_sort('Size','size','light')?></th>
   <!-- <th align="left">&nbsp;Actions</th> -->
   </tr>
   <?
     if($res) light_show_filelist();
     else echo '<tr height=50 bgcolor="#eeeeee"><td colspan=3><script><!-- // a little fix for "refresh" button ;)
     if(document.getElementById) document.getElementById("refresh_a").href = "index.php?DIR='.rawurlencode($_SESSION['DIR']).'&rand='.rand().'"; 
     //-->
     </script><div align="center">Sorry, the directory could not be opened.'.reason().'<br><br>Hit <a href="index.php?DIR='.rawurlencode($_SESSION['DIR']).'&amp;rand='.rand().'">refresh</a> to continue</div></td></tr>';
  ?>
  </table>

  <script> if(document.getElementById){ document.getElementById('sort_menu').innerHTML =  '<div style="float: left;<?=(sizeof($dirs)+sizeof($files)>0 ? '' : ' visibility: hidden;')?>" id="all_div"><input type="checkbox" name="all" id="all" align="left" onclick="select_all(this);"><label for="all"> (all)</label></div>' + document.getElementById('sort_menu').innerHTML; }</script>
  <?=($pg)?>
